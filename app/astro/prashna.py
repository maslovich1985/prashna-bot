...
def judgment_factors(chart: PrashnaChart) -> list[str]:
    ...
    out.append(f"Лагна: {C.SIGNS[asc]} {dms(chart.asc_deg)} — качество знака: {C.SIGN_QUALITY[asc]}.")
    if chart.asc_deg < 3:
        out.append("Лагна в самом начале знака — дело ещё не созрело / рано судить, вопрос преждевременный.")
    elif chart.asc_deg > 27:
        out.append("Лагна в конце знака — дело уже фактически решено или уходит из рук, ситуация на исходе.")

    out.append(f"Владыка лагны {lagna_lord} — {ll.house}-й дом, {ll.dignity}"
               + (", ретроградный" if ll.retro else "") + (", сожжён" if ll.combust else "") + ".")
    out.append(f"Владыка {qh}-го дома (дома вопроса) {qh_lord} — {ql.house}-й дом, {ql.dignity}"
               + (", ретроградный" if ql.retro else "") + (", сожжён" if ql.combust else "") + ".")

    # Связь лагнеша и владыки дома вопроса
    if lagna_lord == qh_lord:
        out.append("Владыка лагны и владыка дома вопроса — одна планета: сильная связь вопрошающего с предметом вопроса (благоприятно).")
    else:
        same_sign = ll.sign == ql.sign
        mutual = (ll.sign == ql.sign) or (abs(houses_between(ll.sign, ql.sign) - 7) == 0)
        if same_sign:
            out.append("Владыка лагны и владыка дома вопроса в соединении — прямая связь, результат вероятен.")
        elif houses_between(ll.sign, ql.sign) == 7:
            out.append("Владыка лагны и владыка дома вопроса в оппозиции (7/7) — связь есть, но через противостояние/переговоры.")
        elif ll.sign == ql.sign:
            out.append("Планеты связаны.")
        else:
            out.append("Прямой связи (соединение/оппозиция) между владыкой лагны и владыкой дома вопроса нет — событие требует дополнительных усилий.")
    ...
    out.append(f"Луна: {C.SIGNS[moon.sign]}, {moon.house}-й дом, накшатра {C.NAKSHATRAS[moon.nakshatra]} "
               f"(владыка {moon.nak_lord}), {moon.dignity}.")
    moon_aspected_by = [n for n, items in chart.aspects.items()
    ...
    if moon_aspected_by:
        ...
        if ben:
            out.append("Луну аспектируют благодетели: " + ", ".join(ben) + " — поддержка, благоприятный исход.")
        if mal:
    ...
    if occupants:
        ...
        out.append(f"В {qh}-м доме находятся: " + ", ".join(occupants)
    ...
    out.append(f"Аруда лагна (AL): {C.SIGNS[chart.aruda_lagna]} — {al_house}-й дом от лагны "
               "(как ситуация выглядит со стороны).")
    qa_house = houses_between(chart.asc_sign, chart.question_house_aruda)
    out.append(f"Аруда {qh}-го дома (A{qh}): {C.SIGNS[chart.question_house_aruda]} — {qa_house}-й дом от лагны.")
    ...
    for n, p in chart.planets.items():
        ...
        if rel in (8, 12) and n in C.MALEFICS:
            out.append(f"{n} (вредитель) в {rel}-м доме от дома вопроса — фактор разрушения/утечки результата.")

    # Даша
    out.append(f"Вимшоттари на момент вопроса: {chart.dasha['строка']} "
...
def render_chart_text(chart: PrashnaChart) -> str:
    ...
    lines.append(f"Время вопроса: {chart.when_local.strftime('%d.%m.%Y %H:%M:%S')} (местное) / "
                 f"{chart.when_utc.strftime('%H:%M:%S')} UTC")
    lines.append(f"Аянамша: {chart.ayanamsa_value:.4f}°, дома — целый знак")
    lines.append("")
    lines.append(f"Лагна: {C.SIGNS[chart.asc_sign]} ({C.SIGNS_SANSKRIT[chart.asc_sign]}) "
                 f"{dms(chart.asc_deg)}, накшатра {C.NAKSHATRAS[chart.asc_nak]} пада {chart.asc_pada}, "
    ...
    for h in range(1, 13):
        ...
        lines.append(f"  {h:>2d} {C.SIGNS[hs]:10s} влад. {chart.house_lords[h]}"
...
def render_short(chart: PrashnaChart) -> str:
    ...
    for name in C.PLANETS:
        ...
        rows.append(f"{C.PLANET_SHORT[name]}{mark} {C.SIGNS[pp.sign][:3]} {int(pp.deg):02d}° д{pp.house}")
    return (
        ...
        f"<b>Панчанга:</b> {chart.panchanga['титхи']}, накшатра Луны {chart.panchanga['накшатра_луны']}"
    )
